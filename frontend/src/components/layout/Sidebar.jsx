              ...
              {({ isActive }) => (
                <>
                  <div className="flex items-center space-x-3">
                    <Icon className={`h-4 w-4 transition-colors ${isActive ? 'text-brand-600' : 'text-slate-400 group-hover:text-slate-600'}`} />
                    <span>{item.label}</span>
                  </div>
                  {isActive && <ChevronRight className="h-3.5 w-3.5 text-brand-600" />}
                </>
              )}
            </NavLink>
          );
      ...
      <div className="px-4 pt-4 border-t border-slate-100">
        <div className="p-3 rounded-xl bg-brand-50/60 border border-brand-100 flex items-center space-x-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-brand-600 font-semibold truncate">{user.roleDisplayName}</p>
          </div>
